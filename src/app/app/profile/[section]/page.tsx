...
const SECTION_CONFIG: Record<SupportedSection, SectionConfig> = {
  notifications: {
    ...
    badge: "Operational",
    description:
      "Control which care events interrupt the shift and which ones can wait for a summary.",
    ...
    toggles: [
      {
        ...
        description:
          "Immediate alerts for high-severity incidents and safeguarding issues.",
      },
      {
        key: "overdueTasks",
        label: "Overdue tasks",
        description: "Prompt staff when tasks pass their due time.",
      },
      {
        key: "handoverReminders",
        label: "Shift handover reminders",
        description: "Send prompts before handover windows close.",
      },
      {
        key: "dailySummary",
        label: "Daily summary",
        description: "Receive a calmer summary view for lower priority events.",
      },
    ],
  },
  pin: {
    ...
    badge: "Security",
    description:
      "Quick unlock helps on shared floor devices while keeping sessions protected.",
  ...
  appearance: {
    ...
    badge: "Experience",
    description:
      "Tune the interface for speed, contrast, and calmer shift handovers.",
    icon: <Moon className="h-5 w-5 text-primary-600" />,
    toggles: [
      {
        key: "denseMode",
        label: "Compact lists",
        description: "Show more residents and tasks per screen.",
      },
      {
        key: "reduceMotion",
        label: "Reduced motion",
        description: "Tone down movement and transitions.",
      },
    ],
  },
  privacy: {
    ...
    badge: "Protected",
    description:
      "Protect resident data with scoped device habits and safer sign-in defaults.",
    icon: <Shield className="h-5 w-5 text-primary-600" />,
    toggles: [
      {
        key: "biometricPrompt",
        label: "Biometric prompt on resume",
        description: "Ask for device auth when returning to the app.",
      },
      {
        key: "shareDiagnostics",
        label: "Share diagnostics with support",
        description:
          "Include safe technical details when support is requested.",
      },
    ],
  },
  help: {
    ...
    badge: "Support",
    description:
      "Set how support should reach your team and what to include in help requests.",
    icon: <Headphones className="h-5 w-5 text-primary-600" />,
    toggles: [
      {
        key: "includeDiagnostics",
        label: "Include diagnostics in tickets",
        description: "Attach environment details to speed up troubleshooting.",
      },
      {
        key: "allowCallbacks",
        label: "Allow callback requests",
        description: "Let support arrange a return call for urgent issues.",
      },
    ],
  },
  settings: {
    ...
    badge: "Admin",
    description:
      "Keep day-to-day workflows stable while tailoring defaults for your organisation.",
...
export default function ProfileSectionPage({
  ...
  const [supportChannel, setSupportChannel] = useState<"email" | "phone">(
    "email",
  );
  ...
  useEffect(() => {
    ...
    setDefaultTaskPriority(
      nextOrgSettings.app?.defaultTaskPriority || "medium",
    );
  ...
  const setPreferenceToggle = (
    group: keyof PreferencesShape,
    key: string,
    value: boolean,
  ) => {
  ...
  const setOrganisationToggle = (
    key: keyof NonNullable<OrgSettingsShape["app"]>,
    value: boolean,
  ) => {
  ...
  const saveSection = async () => {
    if (!user) {
      setError(
        "Your session is not ready yet. Please reopen your profile and try again.",
      );
      return;
    }
    ...
    try {
      ...
      const nextPreferences: PreferencesShape = {
        ...
        privacy: {
          ...(preferences.privacy || {}),
          autoLockMinutes: Math.max(
            1,
            Number.parseInt(autoLockMinutes || "15", 10) || 15,
          ),
        },
        support: {
          ...(preferences.support || {}),
          ...
          <div className="space-y-3">
            {config.toggles?.map((toggle) => (
              <ToggleRow
                ...
                onChange={(checked) =>
                  setPreferenceToggle("notifications", toggle.key, checked)
                }
              />
            ))}
          </div>
        ...
        return (
          <div className="space-y-4">
            <ToggleRow
              ...
              onChange={(checked) =>
                setPreferenceToggle("pin", "quickUnlockEnabled", checked)
              }
            />
            <Input
            ...
            <div className="space-y-3">
              {config.toggles?.map((toggle) => (
                <ToggleRow
                  ...
                  description={toggle.description}
                  checked={
                    !!toggleValue.appearance[
                      toggle.key as keyof typeof toggleValue.appearance
                    ]
                  }
                  onChange={(checked) =>
                    setPreferenceToggle("appearance", toggle.key, checked)
                  }
                />
              ))}
            </div>
            ...
            <div className="space-y-3">
              {config.toggles?.map((toggle) => (
                <ToggleRow
                  ...
                  checked={
                    !!toggleValue.privacy[
                      toggle.key as keyof typeof toggleValue.privacy
                    ]
                  }
                  onChange={(checked) =>
                    setPreferenceToggle("privacy", toggle.key, checked)
                  }
                />
              ))}
            </div>
        ...
        return (
          <div className="space-y-4">
            <SegmentedChoice
              ...
              onChange={(value) =>
                setSupportChannel(value as "email" | "phone")
              }
              options={[
                ["email", "Email"],
                ["phone", "Phone"],
            ...
            <div className="space-y-3">
              {config.toggles?.map((toggle) => (
                <ToggleRow
                  ...
                  checked={
                    !!toggleValue.support[
                      toggle.key as keyof typeof toggleValue.support
                    ]
                  }
                  onChange={(checked) =>
                    setPreferenceToggle("support", toggle.key, checked)
                  }
                />
              ))}
            </div>
        ...
        return (
          <div className="space-y-4">
            <ToggleRow
              ...
              onChange={(checked) =>
                setOrganisationToggle("handoverReminders", checked)
              }
            />
            <SegmentedChoice
            ...
            <SegmentedChoice
              ...
              onChange={(value) =>
                setResidentListView(value as "cards" | "compact")
              }
              options={[
                ["cards", "Cards"],
                ["compact", "Compact"],
  ...
  return (
    <PageContainer
      header={<MobileHeader title={config.title} backHref="/app/profile" />}
    >
      {isLoading ? (
        <Card padding="lg">
        ...
        <div className="space-y-4">
          <Card
            className="border-white/70 bg-gradient-to-br from-white via-slate-50 to-primary-50 shadow-xl"
            padding="lg"
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="mb-3 inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-primary-100">
                  {config.icon}
                </div>
