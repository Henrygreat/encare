...
import { ArrowLeft, ClipboardList, User, Calendar, Flag, Users } from 'lucide-react'
...
import type { TaskInsert, TaskPriority, Resident, User as UserType } from '@/lib/database.types'
...
export default function NewTaskPage() {
  ...
  // Load residents and staff for dropdowns
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        if (residentsResult.data) setResidents(residentsResult.data as Resident[])
        if (staffResult.data) setStaff(staffResult.data as UserType[])
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({
        general: err instanceof Error ? err.message : 'Unable to create task. Please try again.',
      })
    } finally {
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-slate-600">Assign a task to your team or a specific resident.</p>
        </div>
      </div>
