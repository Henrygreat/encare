...
import { Clock, AlertCircle, CheckCircle, ChevronRight, User } from 'lucide-react'
...
const priorityConfig: Record<TaskPriority, { color: 'default' | 'info' | 'warning' | 'danger'; label: string }> = {
...
const statusConfig: Record<TaskStatus, { color: 'default' | 'success' | 'warning' | 'danger' | 'info' | 'purple'; label: string }> = {
...
export function TaskCard({
  ...
  return (
    <div
      className={cn(
        'bg-white rounded-card border p-4 transition-all',
        isOverdue ? 'border-care-red bg-red-50/50' : 'border-surface-200',
        onClick && 'cursor-pointer active:scale-[0.99] hover:shadow-card'
      )}
      onClick={onClick}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            {isOverdue && (
              <AlertCircle className="h-4 w-4 text-care-red flex-shrink-0" />
            )}
        ...
        <div className="flex flex-col items-end gap-2">
          <div className={cn(
            ...
            isOverdue ? 'text-care-red font-medium' : 'text-gray-500'
          )}>
            <Clock className="h-4 w-4" />
            <span>
              {isOverdue ? formatRelativeTime(task.due_at) : formatTime(task.due_at)}
            </span>
          </div>
      ...
      {(onComplete || onSnooze || onEscalate) && task.status === 'pending' && (
